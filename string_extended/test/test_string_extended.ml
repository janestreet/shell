...
module%test [@name "collate"] _ = struct
  ...
  module%test [@name "collate is a strict total order"] _ = struct
    let%expect_test "collate is irreflexive" =
      quickcheck_m (module String) ~f:(fun s -> require (not (s <! s)))
    ;;
    ...
    let%expect_test "collate is transitive" =
      quickcheck_m
        ...
        ~f:(fun (a, b, c) -> require ((a <! b && b <! c) ==> (a <! c)))
    ;;
    ...
    let%expect_test "collate is connected" =
      quickcheck_m
        ...
        ~f:(fun (a, b) -> require (String.( <> ) a b ==> (a <! b || b <! a)))
    ;;
  end
end

module%test [@name "edit_distance"] _ = struct
...
module%test [@name "unescaped_exn"] _ = struct
  ...
  let%expect_test "unescaped_exn" =
    ...
    (* hex escape *)
    require_equal (module String) (unescaped_exn "\\xff") "\xff";
    (* strict, illegal escape *)
    require_does_raise (fun () -> unescaped_exn "\\a");
    [%expect
      {|
      (Invalid_argument
