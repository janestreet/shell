...
(** [collate s1 s2] sorts string in an order that's usually more suited for human
    ...
    - Numerical chunks are compared based on the values of the represented ints and the
      number of trailing zeros.

    It is a total order. *)
val collate : string -> string -> int

(** [unescaped_exn s] is the inverse operation of [escaped]: it takes a string where all
    the special characters are escaped following the lexical convention of OCaml and
    returns an unescaped copy. The [strict] switch is on by default and makes the function
    treat illegal backslashes as errors. When [strict] is [false] every illegal backslash
    except escaped numeral greater than [255] is copied literally. The aforementioned
    numerals still raise errors. This mimics the behaviour of the ocaml lexer. *)
...
(** [squeeze str] reduces all sequences of spaces, newlines, tabs, and carriage returns to
    single spaces. *)
...
(** [word_wrap ~soft_limit s]
    ...
    Wraps the string so that it fits the length [soft_limit]. It doesn't break words
    unless we go over [hard_limit].
...
(** Gives the Levenshtein distance between 2 strings, which is the number of insertions,
    ...
    [transpose] argument, it also considers transpositions (Damerau-Levenshtein distance). *)
